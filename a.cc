...
template<class T, int SZ> struct OffBIT2D {
	...
	int cnt[SZ], st[SZ];
	vector<int> val[SZ]; vector<T> bit[SZ]; // store all BITs in single vector
	void init() { assert(!mode); mode = 1;
		...
		for (int i = 0; i < SZ; i++) val[i].resize(cnt[i] + 1), bit[i].resize(cnt[i] + 1);
		for (int i = 0; i < SZ; i++) lst[i] = 0;
		reverse(all(todo));
		for (auto t : todo) for (int x = t.first; x < SZ; x += x&-x)
			if (lst[x] != t.second) lst[x] = t.second, val[x][cnt[x]--] = t.second;
	}
	int rank(int x, int y, int l, int r) {
		return upper_bound(all(val[x])-begin(val)-l); }
